...
const DeviceSurface = ({
    ...
    useEffect(() => {
        const fetchLogData = async () => {
            const response = await fetch(`https://io.adafruit.com/api/v2/${ADAFRUIT_IO_USERNAME}/feeds/${deviceID}/data`, {
                headers: {
    ...
    const data = {
        labels: logData.map(log => new Date(log.created_at).toLocaleTimeString()),
        datasets: [{
            label: 'Value',
        ...
        >
            <Stack direction="column" spacing={4}>
                ...
                >
                    ...
                    <Box maxWidth={300}>
                        <Typography level="h3" noWrap>{name} ({type})</Typography>
                        <Typography
                            level="body-sm"
            ...
            <DeviceDetailModal open={openDetailModal} onClose={() => setOpenDetailModal(false)} deviceId={id} />
        </Card>
    );
};
